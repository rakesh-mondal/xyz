...
export function SnapshotPolicyModal({ open, onClose, onSave, mode, initialPolicy }: SnapshotPolicyModalProps) {
  const { toast } = useToast()
  
  // Form state
  const [maxSnapshots, setMaxSnapshots] = useState(initialPolicy?.maxSnapshots || "5")
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const policy = {
      name: customName || "Snapshot Policy",
      description,
      maxSnapshots: parseInt(maxSnapshots),
      minute,
      hour,
      dayOfMonth,
      month,
    ...
    toast({
      ...
      description: `Snapshot policy "${policy.name}" has been ${mode === "add" ? "created" : "updated"} successfully.`,
    })
  }
      ...
      <DialogContent className="p-0 bg-white max-w-[60vw] max-h-[85vh] w-[60vw] h-[85vh] overflow-hidden flex flex-col">
        <DialogHeader className="flex-shrink-0 p-6 border-b">
          <DialogTitle className="text-2xl font-semibold">
            {mode === "add" ? "Create Snapshot Policy" : "Edit Snapshot Policy"}
          </DialogTitle>
        </DialogHeader>
          ...
          <div className="flex-1 overflow-y-auto" style={{ scrollBehavior: 'auto', overflowAnchor: 'none' }}>
            <form onSubmit={handleSubmit} ref={formRef} className="space-y-6">
              ...
              <div>
                ...
                <Select value={maxSnapshots} onValueChange={setMaxSnapshots} required>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
